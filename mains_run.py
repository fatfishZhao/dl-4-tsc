...
threads = []
for gpu_id in range(2,9):
    thread = myThread(workQueue, gpu_id)
    thread.start()
    threads.append(thread)
for dataset in datasets:
    for i in range(start_i,end_i):
        workQueue.put('/data2/zyx/software/anaconda3/bin/python main.py mts_archive '+dataset+ ' resnet_torch _itr_'+str(i))
for i in range(8):
    workQueue.put('break')
print('game over')
...
for dataset in tqdm.tqdm(datasets):
    ...
    for i in range(start_i,end_i):
        try:
            best_df = pd.read_csv(os.path.join('/data3/zyx/project/tsc/data/results/resnet_torch', 'mts_archive_itr_'+str(i), dataset, 'best_val.csv'),
                                  index_col=0)
            best_df.index = range(best_df.shape[0])
            best_list.append(best_df['val_acc'][0])
        except:
